...
class Response {
  ...
  static async serverError(req, res, error) {

    const logFolderPath = `logs/errors`;
    const logFileName = `error(${format(new Date(), 'yyyy-MM')}).json`;

    const fileExists = await Response.checkFileExists(logFolderPath, logFileName);
  ...
  static validationError(res, errors) {  /* untuk handle validasi model & validasi "espress-validator" */
